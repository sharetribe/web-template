...
export const FETCH_CURRENT_USER_TRANSACTIONS_REQUEST =
  'app/user/FETCH_CURRENT_USER_TRANSACTIONS_REQUEST';
...
export const FETCH_CURRENT_USER_TRANSACTIONS_SUCCESS =
  'app/user/FETCH_CURRENT_USER_TRANSACTIONS_SUCCESS';
...
const fetchCurrentUserTransactionsRequest = () => ({
...
export const fetchCurrentUserTransactions = () => async (dispatch, getState, sdk) => {
  ...
  try {
    ...
    const totalPages = Math.ceil(transactions.meta.totalItems / 100);

    for (let page = 2; page <= totalPages && page <= 5; page++) {
      const extraTransactions = await fetchPage(page);
      transactions.data = transactions.data.concat(extraTransactions.data);
      transactions.included = transactions.included.concat(extraTransactions.included);
    }
    ...
    dispatch(fetchCurrentUserTransactionsError(error));
    throw error; // Throw error to handle in .catch() in component
  }
};
