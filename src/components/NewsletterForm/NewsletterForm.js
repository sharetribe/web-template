...
const NewsletterForm = ({ className, disclaimerText = '', okMsg, errorMsg }) => {
  ...
  const onSubmit = async e => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)) {
      setMessage({
        type: 'error',
        text: intl.formatMessage({ id: 'NewsletterForm.invalidEmail' }),
      });
      return;
    }
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: intl.formatMessage({ id: 'NewsletterForm.networkError' }),
      });
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <form onSubmit={onSubmit} className={className}>
      ...
      {message && (
        <div className={message.type === 'ok' ? css.msgOk : css.msgError}>{message.text}</div>
      )}
