...
const IconArrowHead = props => {
  ...
  const isMedium = size === SIZE_MEDIUM;

  const sizes = {
    big: {width:11,height:15},
    medium: {width:10,height:14},
    small: {width:9,height:13},
  }

  const applySize = sizes[size];
...
IconArrowHead.propTypes = {
  ...
  size: oneOf([SIZE_BIG, SIZE_SMALL, SIZE_MEDIUM]),
};
