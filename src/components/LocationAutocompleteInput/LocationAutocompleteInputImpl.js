...
function LocationPredictionsList(props) {
  const {
    rootClassName,
    className,
    children,
...
class LocationAutocompleteInputImplementation extends Component {
  ...
  handlePredictionsSelectMove(touchCoordinates) {
    this.setState((prevState) => {
      const { touchStartedFrom } = prevState;
  ...
  render() {
    ...
    const geocoderVariant = getGeocoderVariant(config.maps.mapProvider);
    const { GeocoderAttribution } = geocoderVariant;
...
function LocationAutocompleteInputImpl(props) {
