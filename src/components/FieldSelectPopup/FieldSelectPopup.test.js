...
describe('FieldSelectPopup', () => {
  ...
  describe('keyboard navigation and ARIA', () => {
    ...
    it("announces the disabled placeholder as not selected, instead of announcing the placeholder's own visible text as if it were the current value", () => {
      render(<FormComponent onSubmit={noop} />);

      expect(
        screen.getByRole('button', { name: 'Start time FieldSelectPopup.screenreader.notSelected' })
      ).toBeInTheDocument();
      expect(
        screen.queryByRole('button', { name: 'Start time Choose start time' })
