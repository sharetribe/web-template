...
const ErrorMessage = props => {
  ...
  return error ? (
    <p className={css.error}>
      {listingNotFound ? (
        <FormattedMessage id="RequestQuotePage.listingNotFoundError" />
      ) : noTransactionProcessAlias ? (
        <FormattedMessage id="RequestQuotePage.requestQuoteErrorNoProcess" />
      ) : (
        <FormattedMessage id="RequestQuotePage.requestQuoteError" />
      )}
    </p>
  ) : null;
};
