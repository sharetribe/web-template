...
import * as searchMapSubMapbox from './SearchMapWithMapbox';
import * as searchMapSubGoogleMaps from './SearchMapWithGoogleMaps';
import ReusableMapContainer from './ReusableMapContainer';
import css from './SearchMapSub.module.css';

const REUSABLE_MAP_HIDDEN_HANDLE = 'reusableMapHidden';

const getSearchMapSubVariant = mapProvider => {
  const isGoogleMapsInUse = mapProvider === 'GOOGLE_MAPS';
  return isGoogleMapsInUse ? searchMapSubGoogleMaps : searchMapSubMapbox;
};
const getSearchMapSubVariantHandles = mapProvider => {
  const searchMapSubVariant = getSearchMapSubVariant(mapProvider);
  return {
    labelHandle: searchMapSubVariant.LABEL_HANDLE,
    infoCardHandle: searchMapSubVariant.INFO_CARD_HANDLE,
  };
};
const getFitMapToBounds = mapProvider => {
  const searchMapSubVariant = getSearchMapSubVariant(mapProvider);
  return searchMapSubVariant.fitMapToBounds;
};
const getSearchMapSubVariantComponent = mapProvider => {
  const searchMapSubVariant = getSearchMapSubVariant(mapProvider);
  return searchMapSubVariant.default;
};
...
export class SearchMapSubComponent extends Component {
  constructor(props) {
    super(props);
  ...
  onMapClicked(e) {
    ...
    const variantHandles = getSearchMapSubVariantHandles(this.props.config.maps.mapProvider);
  ...
  render() {
    ...
    const SearchMapSubVariantComponent = getSearchMapSubVariantComponent(mapProvider);

    return getSearchMapSubVariant(mapProvider).isMapsLibLoaded() ? (
...
SearchMapSubComponent.defaultProps = {
  id: 'SearchMapSub',
  className: null,
  rootClassName: null,
...
SearchMapSubComponent.propTypes = {
  id: string,
  className: string,
...
const SearchMapSub = props => {
  const config = useConfiguration();
  const routeConfiguration = useRouteConfiguration();
  const history = useHistory();
  return (
    <SearchMapSubComponent
      config={config}
