...
const getAvailableTimeRangesForExceptions = (exceptions, timeZone) => {
  ...
  const initialRangeCollection = isDateSameOrAfter(
    nextBoundary,
    sortedExceptions[0].attributes.start
  )
    ? { prevEnd: null, ranges: [] }
  ...
  return isDateSameOrAfter(endOfLastException, lastBoundary)
...
const getAvailableStartTimes = (date, timeRangesOnSelectedDate, intl, timeZone) => {
  ...
  const startOfDate = getStartOf(date, 'day', timeZone);
...
const startTimeOrStartOfSelectedEndDay = (selectedStartTimeAsDate, startOfSelectedEndDay) =>
  isDateSameOrAfter(selectedStartTimeAsDate, startOfSelectedEndDay)
...
const timeRangeEndOrNextMidnight = (timeRangeEnd, dayAfterSelectedEnd) =>
  isDateSameOrAfter(dayAfterSelectedEnd, timeRangeEnd) ? timeRangeEnd : dayAfterSelectedEnd;
...
const isOutsideRange = (exceptionStartDate, selectedTimeRange, timeZone) => day => {
  ...
  const startDate = getStartOf(exceptionStartDate, 'day', timeZone);
  ...
  return !(isDateSameOrAfter(localizedDay, startDate) && isDateSameOrAfter(endDate, localizedDay));
};
...
const Next = props => {
  ...
  return isDateSameOrAfter(
    nextMonthDate,
    endOfAvailabilityExceptionRange(timeZone, TODAY)
  ) ? null : (
...
const Prev = props => {
  const { currentMonth, timeZone } = props;
  const prevMonthDate = getStartOf(currentMonth, 'month', timeZone, -1, 'months');
