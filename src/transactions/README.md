...
- Confirm UX: redirect / async (`confirmPayment` + return URL to `/l/:slug/:id/checkout/return`).
  [CheckoutPageRedirectReturn](../containers/CheckoutPage/CheckoutPageRedirectReturn/CheckoutPageRedirectReturn.js)
  resumes confirm-only checkout; card checkout does not mount that logic.
...
- Webhooks: push payments are often async. The browser return path is only a partial completion
  step. If the customer authorizes at the bank/app but never returns, delayed **expire-payment** can
  move the transaction to `payment-expired` and run `stripe-refund-payment` — so a successful Stripe
...
6. **Account deletion** — Update `server/api/delete-account.js` for Stripe-related states (see
   above).
7. **Multi-method checkout** — When more than one method is offered, checkout stores the selected
   method id in `protectedData.checkoutPaymentMethod`. If your process needs to know which payment
   method was used in a later stage, read it from there.
