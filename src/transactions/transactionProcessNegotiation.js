...
export const getTransitionsWithMatchingOffers = (transitions, offers) => {
  ...
  if (isValidOffersArray) {
    const transitionsWithOffers = [...transitions];
    let offerIndex = 0;
    for (let i = 0; i < transitionsWithOffers.length; i++) {
      const transition = transitionsWithOffers[i];

      if (offerTransitions.includes(transition.transition)) {
        transition.quoteInSubunits = offers[offerIndex]?.quoteInSubunits;
        offerIndex++;
      }
    }
