...
module.exports = function(proxy, allowedHost) {
  ...
  return {
    ...
    setupMiddlewares(middlewares, devServer) {
      // Run user proxy setup once (registers routes on devServer.app)
      if (fs.existsSync(paths.proxySetup)) {
        require(paths.proxySetup)(devServer.app);
      }
      // Keep evalSourceMapMiddleware before other middlewares so we can fetch
      // source contents from webpack for the error overlay
      middlewares.unshift(evalSourceMapMiddleware(devServer));
      // Redirect to `PUBLIC_URL` or `homepage` from `package.json` if url not match
      middlewares.push(redirectServedPath(paths.publicUrlOrPath));
      // No-op service worker to reset any previous service worker for the same host:port
