...
module.exports = function(api, opts, env) {
  ...
  if (useAbsoluteRuntime) {
    // Resolve @babel/runtime from the project root to avoid nested node_modules paths
    // This ensures we get the root-level @babel/runtime, not one nested in babel-preset-react-app
    ...
    if (require('fs').existsSync(runtimePackageJson)) {
      ...
      // Fallback to require.resolve, but try project root first
      try {
        absoluteRuntimePath = path.dirname(
          require.resolve('@babel/runtime/package.json', { paths: [projectNodeModules] })
        );
      } catch (e) {
        // Final fallback
