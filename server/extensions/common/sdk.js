...
exports.integrationSdk = flexIntegrationSdk.createInstance({
  ...
  commandLimiter: flexIntegrationSdk.util.createRateLimiter(
    REACT_APP_ENV === 'production'
      ? flexIntegrationSdk.util.prodCommandLimiterConfig
      : flexIntegrationSdk.util.devCommandLimiterConfig
  ),
  queryLimiter: flexIntegrationSdk.util.createRateLimiter(
    REACT_APP_ENV === 'production'
      ? flexIntegrationSdk.util.prodQueryLimiterConfig
      : flexIntegrationSdk.util.devQueryLimiterConfig
  ),
});

exports.getAuthInfo = req => {
  const userToken = getUserToken(req);
  ...
  return sdk
    .authInfo()
    .then(info => info.scopes)
    .catch(error => {
      log.error(error, 'get-auth-info-failed', error.data);
      return null;
    });
};
